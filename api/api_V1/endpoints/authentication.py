...
async def login(
    ...
    duser = await get_user_by_email(db, user.email)
    if not dbuser or dbuser.check_password(user.password):
        raise HTTPException(
...
@router.post(
    "/users", response_model=UserInResponse, tags=["authentication"],
